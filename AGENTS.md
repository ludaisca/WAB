...
- **Campaign sends create CRM records**: a successfully delivered campaign recipient now upserts a `Contact`/`WAChat`/`WAMessage` (`direction: OUTBOUND`, `messageType: "template"`) and tags both the contact and chat with `Campaña: <campaign name>` (`lib/workers/campaign-worker.ts`) — so campaign leads show up in `/whatsapp/contactos` and the chat list, not just in `WACampaignRecipient`. A recipient that fails to send leaves no CRM trace (no phantom contact). **Known gap**: the `WAMessage.body` stored for this record is a cryptic placeholder (`Plantilla: <name> — <param1>, <param2>...`), not the actual rendered template text — see the Backlog section at the end of this file.
...
Prioritized by the user; see the reasoning below each item before picking one up. Remove an item from this list once it's actually implemented and merged.

1. **Bot replies after a campaign send lack context.** When a bot-managed chat receives a reply to a campaign template, `bot-worker.ts`'s `RECENT` memory mode does surface *something* from that send — but it's `WAMessage.body` as written by `campaign-worker.ts`, which is a cryptic placeholder (`Plantilla: <name> — <param1>, <param2>, ...`), not the actual template text the customer read, and it's dropped entirely under `memoryType: "NONE"`/`"SUMMARY"`. The bot ends up replying "blind" to a lead who just received a specific marketing offer. Fix direction: have `campaign-worker.ts` build the real rendered template text (substitute `{{n}}` in `WATemplate.components` body with the recipient's `parameters`, similar to what `<TemplatePreview>` already does for display) into `WAMessage.body` instead of the placeholder, and/or have `bot-worker.ts` always inject a one-line "this chat started from campaign X, template Y" system note when the most recent outbound message came from a campaign, regardless of `memoryType`. **Priority: highest** — affects live bot conversation quality for every campaign already sent, on a feature that just shipped this session (campaign→CRM attribution), narrow and concrete fix.
2. **Contacts module is missing/losing data.** `whatsapp/contactos/page.tsx` already has Nombre/Estado/Etiquetas/Notas/Última actividad columns, but: (a) `ingest-message.ts:48-52` overwrites `Contact.name` on *every* inbound message with `msg.contactName`, which falls back to the raw phone number when WhatsApp doesn't send a profile name on that particular payload — a good name can regress to a phone number later. (b) There's no visible phone number or account column even though the page already has a per-account filter (multiple accounts' contacts are mixed in one table with no way to tell them apart at a glance). Needs a decision on what "algunos más" datos the user actually wants surfaced before scoping the fix — ask before starting. **Priority: second** — contained, no architecture questions, but needs a quick confirmation of exact fields wanted.
3. **Bot system prompt "normalizer."** `bot.systemPrompt` (`bot-worker.ts:86`) is inserted as raw user text with zero sanitization. Two concrete failure modes to guard against: (a) for `WABot`, a careless/adversarial prompt can attempt to override the model's later system messages (RAG context, memory summary) via prompt injection — no contract to "break" today but quality/safety risk; (b) for `WALeadScorerBot`, the score route (`app/api/whatsapp/chats/[chatId]/score/route.ts:112,155`) appends a fixed JSON-contract system message *after* the user's own `scorer.systemPrompt` and then regex-extracts + `JSON.parse()`s the model's reply — a scorer prompt that tells the model to ignore later instructions or respond in a different format will make that `JSON.parse()` throw, failing the whole scoring run. Needs design: what does "normalize" mean here — strip/flag conflicting instructions, wrap the user's prompt in delimiters the model is told to treat as data not new instructions, cap length, or something else? **Priority: third** — real failure mode identified, but needs a design decision before implementation.
4. **Bot "humanization" parameter.** Add some mechanism (typing-indicator delays, splitting a reply into multiple shorter messages, avoiding too-perfectly-formatted responses, etc.) so a bot reply doesn't read as obviously automated. Most open-ended item — needs UX/product design (what exactly should vary, and how much) before any code, and worth a quick note to the user that WhatsApp's Business Messaging Policy expects disclosure of automated/AI messaging in some contexts, so "harder to distinguish from a human" should stay UX-only (tone, pacing) and not become actual deception about *whether* it's automated. **Priority: fourth** — most speculative, most design work, lowest urgency (differentiator, not a bug).
